...
Colors use a from-scratch **RGB color picker** (`createColorPicker`): a swatch preview beside three 0–255 channel sliders, converting to/from hex. Font size and line height use single-value sliders. The **font** uses a `<select>` (`createFontPicker`) of system fonts: since no cross-platform API enumerates installed fonts (`queryLocalFonts` is Chromium/WebView2-only, absent in WebKit), it probes a curated per-OS candidate list (`FONT_CANDIDATES`) with `isFontAvailable` — a canvas `measureText` width comparison against generic baselines — and keeps only those actually installed, plus the generic families. The stored value is a `font-family` string (`"Georgia"`); `primaryFamily` maps a stack back to a single selection. The **Save settings** button serializes the controls to JSON and persists via Go's `SaveConfig`; on startup `LoadConfig` is read back and applied (absent file → stylesheet defaults). Live changes that aren't saved last only for the session.
